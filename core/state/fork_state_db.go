...
func (s *ForkStateDb) populateStateObjectFromFork(addr common.Address) *stateObject {
	...
	bal, nonce, code, err := s.remoteStateProvider.ImportStateObject(addr, s.journal.nextRevisionId)
...
func (s *ForkStateDb) populateSlotFromFork(addr common.Address, hash common.Hash) common.Hash {
	...
	data, err := s.remoteStateProvider.ImportStorageAt(addr, hash, s.journal.nextRevisionId)
	if err != nil {
...
func (s *ForkStateDb) AddBalance(addr common.Address, amount *uint256.Int, reason tracing.BalanceChangeReason) uint256.Int {
	stateObject := s.getStateObject(addr)
	if stateObject == nil {
		stateObject = s.populateStateObjectFromFork(addr)
	}
...
func (s *ForkStateDb) SubBalance(addr common.Address, amount *uint256.Int, reason tracing.BalanceChangeReason) uint256.Int {
	stateObject := s.getStateObject(addr)
	if stateObject == nil {
		stateObject = s.populateStateObjectFromFork(addr)
	}
...
func (s *ForkStateDb) SetNonce(addr common.Address, nonce uint64, reason tracing.NonceChangeReason) {
	stateObject := s.getStateObject(addr)
	if stateObject == nil {
		stateObject = s.populateStateObjectFromFork(addr)
	}
...
func (s *ForkStateDb) SetCode(addr common.Address, code []byte) (prev []byte) {
	stateObject := s.getStateObject(addr)
	if stateObject == nil {
		stateObject = s.populateStateObjectFromFork(addr)
	}

	s.remoteStateProvider.MarkContractDeployed(addr, s.journal.nextRevisionId)
	return stateObject.SetCode(crypto.Keccak256Hash(code), code)
}

func (s *ForkStateDb) SetState(addr common.Address, key, value common.Hash) common.Hash {
	stateObject := s.getStateObject(addr)
	if stateObject == nil {
		stateObject = s.populateStateObjectFromFork(addr)
	}

	s.remoteStateProvider.MarkSlotWritten(addr, key, s.journal.nextRevisionId)
